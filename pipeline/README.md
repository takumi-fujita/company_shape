...
出どころは JPX「東証上場銘柄一覧」。配布形式は `.xlsx`（旧 `.xls` も読める）。
元ファイルはコミットしない（`.gitignore` 済み）。成果物の `industries.csv` だけを追跡する。

東証以外に上場している会社は、この一覧に構造的に載らない。そのため業種が付かず `unknown`／`分類なし` になる（4,290 社中 606 社）。取りこぼしではないので、業種内の順位や中央値比較はフロント側で出さない（`apps/web/lib/industry.ts`）。
